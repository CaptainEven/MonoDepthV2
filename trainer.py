...
class MyStereoTrainer:
    def __init__(self, options):
        ...
        ## --------- Models and parameters to be trained
        # self.parameters_to_train = []

        ## ---------- Set the device
        device = str(find_free_gpu())
        print('Using gpu: {:s}'.format(device))
        device = select_device(device='cpu' if self.opt.no_cuda else device)
        self.device = device
        ...
        ## ----- Combined model: encoder and decoder are combined together
        # self.net = networks.StereoNet(num_layers=self.opt.num_layers,
        #                               pre_trained=self.opt.weights_init == "pretrained",
        #                               num_input_images=1,
        #                               num_ch_enc=[64, 64, 128, 256, 512],
        #                               scales=self.opt.scales,
        #                               num_output_channels=1,
        #                               use_skips=True)
        self.net = networks.EncoderDecoder(num_in_layers=3, encoder='resnet18', pretrained=True)

        self.net.to(self.device)
        # self.parameters_to_train += list(self.net.parameters())

        ## ---------- Optimizer
        self.model_optimizer = optim.Adam(self.net.parameters(), self.opt.learning_rate)
        # self.model_optimizer = optim.SGD(self.net.parameters(), self.opt.learning_rate, momentum=0.0)
        ...
        ## ---------- Dataset
        datasets_dict = {
            "kitti": datasets.KITTIRAWDataset,  # datasets.KITTIRAWDataset
            "kitti_odom": datasets.KITTIOdomDataset,
            "my_experiment": datasets.MyExperimentDataset,
            "apollo_stereo": datasets.ApolloDataset,
            "my_kitti": datasets.MyKittiDataset
        }
        self.dataset = datasets_dict[self.opt.dataset]
        print('Using dataset: {:s}'.format(self.opt.dataset))

        f_path = os.path.join(os.path.dirname(__file__), "splits", self.opt.split, "{}_files.txt")

        train_file_names = readlines(f_path.format("train"))
        val_file_names = readlines(f_path.format("val"))
        ...
        train_dataset = self.dataset(self.opt.data_path,
                                     ...
                                     self.opt.height,
                                     self.opt.width,
                                     img_height=self.opt.image_height,
                                     img_width=self.opt.image_width,
        ...
        self.depth_metric_names = ["de/abs_rel",
                                   "de/sq_rel",
                                   "de/rms",
                                   "de/log_rms",
                                   "da/a1",
    ...
    def run_epoch(self, loss_log_freq=100):
        """
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            self.running_batch_loss += losses["loss"].item()

            if batch_idx != 0 and batch_idx % loss_log_freq == 0:
                print('Average loss of last {:d} batches: {:.3f}.'
                      .format(loss_log_freq, self.running_batch_loss / loss_log_freq))
                self.running_batch_loss = 0.0
                self.save_model()
    ...
    def process_batch(self, inputs):
        ...
        ## @even
        outputs = self.net.forward(inputs["color_aug", 0, 0])

        ## ---------- Build predicted image
        self.generate_images_pred(inputs, outputs)
        # self.gen_imgs_pred(inputs, outputs)
    ...
    def gen_imgs_pred(self, inputs, outputs):
        """
        :param inputs:
        :param outputs:
        :return:
        """
        for scale in self.opt.scales:
            disp = outputs[("disp", scale)]  # left image disparity
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ## Get source pix_coords by epi-polar constraint seching
                flow_field = get_img_field(img=inputs[("color", frame_id, source_scale)], disp=disp)
                pix_coords = 2.0 * flow_field - 1.0
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        :return:
        """
        for scale in self.opt.scales:  # processing each scale
            ...
            ## ---------- Convert scaled inverse depth to depth
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                ## points3d in source images
                pts3d_homo = self.backproject_depth[source_scale].forward(depth, inputs[("inv_K", source_scale)])
    ...
    def compute_depth_losses(self, inputs, outputs, losses):
        ...
        depth_pred = torch.clamp(F.interpolate(depth_pred,
                                               [self.opt.image_height, self.opt.image_width],
                                               mode="bilinear",
                                               align_corners=False),
                                 1e-3,
                                 self.opt.max_depth)
    ...
    def save_model(self):
        ...
        :return:
        """
        save_folder = os.path.join(self.log_path, "models", "epoch_{}".format(self.epoch))
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)

        save_path = os.path.join(save_folder, "{}.pth".format('net'))
...
class StereoTrainer:
    def __init__(self, options):
        ...
        ## ----- Combined model: encoder and decoder are combined together
        self.models['net'] = networks.StereoNet(num_layers=self.opt.num_layers,
                                                pre_trained=self.opt.weights_init == "pretrained",
                                                num_input_images=1,
                                                num_ch_enc=[64, 64, 128, 256, 512],
                                                scales=self.opt.scales,
        ...
        # self.model_optimizer = optim.SGD(self.parameters_to_train, self.opt.learning_rate, momentum=0.0)
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            all_features = self.models['net'].get_encoder_features(all_color_aug)
            ...
            outputs = self.models['net'].get_decoder_depths(features[0])

        else:
            # Otherwise, we only feed the image with frame_id 0 through the depth encoder
            ## @even
            features = self.models['net'].get_encoder_features(inputs["color_aug", 0, 0])
            outputs = self.models['net'].get_decoder_depths(features)

        if self.opt.predictive_mask:
            outputs["predictive_mask"] = self.models["predictive_mask"].forward(features)

        ## ---------- Build predicted image
        self.generate_images_pred(inputs, outputs)  # scled inverse depth([0, 1]) are regressed
        # self.gen_imgs_pred(inputs, outputs)  # scaled disparity([0, 1]) are regressed
    ...
    def predict_poses(self, inputs, features):
        ...
        if self.num_pose_frames == 2:
            ...
            if self.opt.pose_model_type in ["separate_resnet", "posecnn"]:
                pose_inputs = torch.cat([inputs[("color_aug", i, 0)] for i in self.opt.frame_ids if i != "s"],
                                        1)
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                if self.opt.pose_model_type == "posecnn":
                    ...
                    inv_depth = 1.0 / depth  # inverse depth
                    mean_inv_depth = inv_depth.mean(3, True).mean(2, True)

                    ## camera extrinsic matrix T
...
class MergerdTrainer:  # Merge Encoder and Decoder
    def __init__(self, options):
        ...
        if self.use_pose_net:
            if self.opt.pose_model_type == "separate_resnet":
                ...
                self.models["pose"] = networks.PoseCNN(self.num_input_frames
        ...
        train_dataset = self.dataset(self.opt.data_path,
                                     train_file_names,
                                     self.opt.height,  # net height
                                     self.opt.width,  # net width
                                     img_height=self.opt.image_height,
                                     img_width=self.opt.image_width,
                                     frame_ids=self.opt.frame_ids,
                                     num_scales=4,
    ...
    def run_epoch(self, log_freq=100):
        ...
        print('Average loss of this epoch is {:.3f}.'.format(self.avg_loss))
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            outputs = self.models["net"].decoder.forward(features)
    ...
    def log_time(self, batch_idx, n_batches, duration, loss):
        ...
        self.lr = self.model_optimizer.param_groups[0]["lr"]
